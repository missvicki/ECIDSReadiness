...
class K12DataGenerator:
    ...
    def __init__(self, ecids_dir="synthetic_data", k12_dir="k12_data"):
        ...
        self.k12_dir.mkdir(exist_ok=True)
    ...
    def load_option_codes(self):
        ...
        df = pd.read_excel("OptionCodes.xlsx", sheet_name="Sheet1")
        ...
        for _, child in self.df_child.iterrows():
            ...
            for school_year in self.school_years:
                ...
                if grade == 'K':
                    ...
                    ed_adjustment = (parent_ed_numeric - 3.5) * 0.05  # Range: -0.125 to +0.125
                    final_prob = base_prob + ed_adjustment
                    final_prob = max(0.2, min(0.95, final_prob))  # Bound between 20%-95%
    ...
    def generate_stuenrlattnd(self):
        ...
        for _, student in self.df_stucore.iterrows():
            ...
            if engagement_score < 20:
                base_attend_rate = random.uniform(0.95, 1.00)
            elif engagement_score < 35:
                base_attend_rate = random.uniform(0.90, 0.95)
            elif engagement_score < 50:
                base_attend_rate = random.uniform(0.85, 0.90)
            else:
                base_attend_rate = random.uniform(0.75, 0.85)

            # Parent education adds modest contextual influence
            # Higher education slightly improves attendance
            ed_adjustment = (parent_ed_numeric - 3.5) * 0.01  # Range: -0.025 to +0.025
            attend_rate = base_attend_rate + ed_adjustment
            attend_rate = max(0.70, min(1.00, attend_rate))
