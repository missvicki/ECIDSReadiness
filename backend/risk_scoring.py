...
class ReadinessRiskScorer:
    ...
    def __init__(self, data_dir="../dashboard-react/public/data/ecids"):
    ...
    def calculate_composite_score(self, risk_df):
        ...
        # Assign risk tier based on composite score (internal use)
        ...
        # Assign support band (user-facing, less stigmatizing)
        # Uses same thresholds but framed as support needs, not risk labels
        risk_df["support_band"] = pd.cut(
            risk_df["composite_risk_score"],
            bins=[0, 24, 35, 50, 100],
            labels=["On Track", "Monitor", "Targeted Support", "Intensive Support"],
            include_lowest=True
        )
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(f"\nSupport Band Distribution (User-Facing):")
    print(risk_data["support_band"].value_counts().sort_index())

    print(f"\nRisk Tier Distribution (Internal):")
    ...
    # Save to CSV
    output_path = Path("../dashboard-react/public/data/ecids") / "risk_scores.csv"
